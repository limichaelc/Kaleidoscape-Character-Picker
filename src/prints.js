...
const ABILITY_EMOJI = {
  [ABILITY_TYPE.STRENGTH]: '<:Strength:952974314561486858>',
  [ABILITY_TYPE.SKILL_DAMAGE]: '<:SkillDamage:952974314393698395>',
  [ABILITY_TYPE.CRITICAL_RATE]: '<:CriticalRate:952974313991049286>',
  [ABILITY_TYPE.FORCE_STRIKE]: '<:ForceStrike:952974314121072700>',
  [ABILITY_TYPE.HP]: '<:HP:952974314167230514>',
  [ABILITY_TYPE.DRAGON_DAMAGE]: '<:DragonDamage:952974314104299560>',
  [ABILITY_TYPE.DRAGON_HASTE]: '<:DragonHaste:952974313668104204>',
  [ABILITY_TYPE.SKILL_HASTE]: '<:SkillHaste:952974314762813490>',
  [ABILITY_TYPE.SKILL_PREP]: '<:SkillPrep:952974314662133790>',
  [ABILITY_TYPE.DEFENSE]: '<:Defense:952974314163015690>',
  [ABILITY_TYPE.CRITICAL_DAMAGE]: '<:CriticalDamage:952974314016243712>',
  [ABILITY_TYPE.RECOVERY_POTENCY]: '<:RecoveryPotency:952974313785536593>',
  [ABILITY_TYPE.DRAGON_TIME]: '<:DragonTime:952974314079150142>',
  [ABILITY_TYPE.STEADY_HITTER]: '<:SteadyHitter:952974314309816351>',
  [ABILITY_TYPE.EASY_HITTER]: '<:EasyHitter:952974314813141052>',
  [ABILITY_TYPE.LUCKY_HITTER]: '<:LuckyHitter:952974314125262968>',
...
async function genAdventurerData(adventurer) {
  const results = await sql`
    ...
    WHERE CONCAT(',', aliases, ',') LIKE CONCAT('%,', ${adventurer}::text, ',%') OR LOWER(name) = ${adventurer.toLower()}
  `;
  if (results.length == 0) {
